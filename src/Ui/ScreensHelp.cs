...
public partial class Screens{	
	void setHelp(int page = 0, bool ignore = false){
		...
		//Just to to avoid rewriting a lot of code
		...
		Keybinds.up.subEvent(l3, false, (s, ck) => {
			content.Scroll++;
		});
		
		Keybinds.down.subEvent(l3, false, (s, ck) => {
			content.Scroll--;
		});
		
		switch(page){
			case 0:
				content.AppendLine("Introduction", Palette.info);
				
				content.AppendLine("AshRadio is a lightweight music player with a terminal-based UI.", null);
				content.AppendLine("It stores songs locally in an internal folder, and does not require internet to work.", null);
				content.AppendLine("You can import songs from local files or from YouTube.", null);
				content.AppendLine("The UI and keybinds to control it is super customizable.", null);
				break;
			case 1:
				content.AppendLine("Concepts", Palette.info);
				
				content.Append("Session", Palette.info);
				content.AppendLine(": the current options for source, order, queue and output device", null);
				...
				content.AppendLine(": the order in which the next song will be chosen: order, shuffle or smart shuffle(dont repeat)", null);
				content.Append("Queue", Palette.info);
				content.AppendLine(": if not empty, next song will be chosen from here instead of the source. There is an additional option to not empty it (this allows for repetition)", null);
				content.Append("Library", Palette.info);
				content.AppendLine(": the whole collection of songs", null);
				content.Append("Authors", Palette.info);
				content.AppendLine(": each song can have any number of authors, and these are created automatically when you change a song's authors", null);
				content.Append("Playlists", Palette.info);
				content.AppendLine(": ordered collections of songs. Duplicates are allowed", null);
				break;
			case 2:
			...
			case 3:
				...
				content.AppendLine("Selection:", null);
			...
			case 5:
				...
				content.AppendLine("You can export songs, whole playlists, all songs by an author or any selection of songs to folders.", null);
